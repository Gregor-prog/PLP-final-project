...
exports.doctorregister = (req, res)=>{
    ...
    if (!email || !phone || !firstname){
        ...
        db.query(`select email from doctors where email = ?`, [email], async(err, result)=>{
            ...
            db.query(`insert into doctors set ?`, {firstname:firstname, lastname:lastname, specialty:specialty, date_of_birth:date_of_birth, email:email, phone:phone,gender:gender, address:address, date_of_birth:date_of_birth,gender:gender, password:hashedpass}, 
                (err, result)=>{
                if(err){
                    console.log(err);
